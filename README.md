...
* Allows editing an arbitrary number of multiresolution modifier-enabled meshes at once through creating a single proxy mesh.
    ...
    * Once the Multires level is frozen, higher levels are erased. Lower levels can be reconstructed with the Multires modifier using the `Unsubdivide` button. To preserve all subdivions, set the level to the highest for your most dense object in the collection.
    * Click the `Restore Lower Levels` button to retore frozen subdivision levels.
